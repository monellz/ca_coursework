...
int main() {
    ...
    for (int i = 0; i < manager.benches.size(); ++i) {
        CacheSimulator<TAG, INDEX, BLOCK, WAY, WriteBack<TAG>, Score<WAY, 0>> s0_cache(WriteAllocate::Yes);
        CacheSimulator<TAG, INDEX, BLOCK, WAY, WriteBack<TAG>, Score<WAY, 8>> s8_cache(WriteAllocate::Yes);
        CacheSimulator<TAG, INDEX, BLOCK, WAY, WriteBack<TAG>, Score<WAY, 16>> s16_cache(WriteAllocate::Yes);
        CacheSimulator<TAG, INDEX, BLOCK, WAY, WriteBack<TAG>, Score<WAY, 32>> s32_cache(WriteAllocate::Yes);
        CacheSimulator<TAG, INDEX, BLOCK, WAY, WriteBack<TAG>, Score<WAY, 64>> s64_cache(WriteAllocate::Yes);
        ...
        for (int j = 0; j < manager.benches[i].access.size(); ++j) {
            s8_cache.access(manager.benches[i].access[j]);
        }
        printf("[%s] [s8_cache] cache_miss: %d/%d=%f\n", manager.benches[i].name.c_str(), perf.cache_miss, perf.total_access, (float)perf.cache_miss / perf.total_access);
        perf.clear();
